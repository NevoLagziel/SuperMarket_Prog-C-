...
int	saveAddressToCompressedFile(const Address* pAddress, FILE* fp)
{
	BYTE houseNum = (BYTE)pAddress->num;
	if (fwrite(&houseNum, sizeof(BYTE), 1, fp) != 1)
		return 0;
	int streetLen, cityLen;
	if (!writeStringToCompressedFile(pAddress->street, fp, "Error write steet\n"))
		return 0;
	if (!writeStringToCompressedFile(pAddress->city, fp, "Error write city\n"))
		return 0;
	return 1;
}

int	loadAddressFromCompressedFile(Address* pAddress, FILE* fp)
{
	BYTE houseNum;
	if (fread(&houseNum, sizeof(BYTE), 1, fp) != 1)
		return 0;
	pAddress->num = (int)houseNum;

	pAddress->street = readStringFromCompressedFile(fp, "Error reading street\n");
	if (!pAddress->street)
		return 0;

	pAddress->city = readStringFromCompressedFile(fp, "Error reading city\n");
	if (!pAddress->city)
	{
